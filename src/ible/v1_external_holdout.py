...
def _load_seed(root: Path, seed_path: Path, tickers: list[str]) -> tuple[dict[str, Any], dict[str, Any], dict[str, str], dict[str, Any]]:
    try:
        seed = json.loads(seed_path.read_text(encoding="utf-8"))
    except (OSError, ValueError) as exc:
        raise RuntimeError(f"V1 external holdout seed unavailable or invalid: {exc}") from exc
    expected_hash = str((seed.get("metadata") or {}).get("content_sha256") or "")
    actual_hash = compute_seed_sha256(seed)
    if not expected_hash or expected_hash != actual_hash:
        raise RuntimeError("V1 external holdout seed integrity SHA-256 mismatch")
    client = SecFsdsClient(root / ".cache" / "sec_fsds_v1_external", "offline-seed@example.invalid")
...
def run_v1_external_holdout(root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    missing = sorted(set(cohort_ids) - set(themes_by_id))
    if missing:
        raise RuntimeError(f"V1 external exposure taxonomy missing themes: {missing}")
    ...
    if str(metadata.get("cutoff") or "") != as_of:
        raise RuntimeError(f"V1 external seed cutoff mismatch: expected {as_of}, got {metadata.get('cutoff')}")
    ...
    dataset_gate_passed = bool(
        ...
        >= int(gates.get("available_company_min", 25))
    ...
    for raw in config.get("scenarios", []):
        ...
        if not dataset_gate_passed:
            ...
            evaluated["reason"] = "V1 외부 홀드아웃 데이터 게이트 미통과"
    ...
    successful_leads = [float(row["lead_time_months"]) for row in positives if row.get("passed") and row.get("lead_time_months") is not None]
    ...
    status = (
        ...
        if passed
        else "V1_EXTERNAL_HOLDOUT_FAILED"
        if dataset_gate_passed
        else "V1_EXTERNAL_HOLDOUT_INSUFFICIENT_DATA"
    )
    ...
    summary = {
        ...
        "scenarios": scenarios,
        "known_limitations": [
            "V0.9.1 점수식·정규화·판정 파일과 외부 홀드아웃 설계 파일을 SHA-256으로 동결합니다.",
            "2023-06-30 점시점 seed는 PC에서 생성하며 GitHub 실행 중 SEC·arXiv·FMP를 호출하지 않습니다.",
            "일부 산업은 기존 상위 분류와 경제적 노출이 겹치지만 날짜·세부 코호트·판정은 별도로 봉인했습니다.",
            "산업 결과 라벨은 2025-12-31까지의 광범위한 산업화 여부를 기준으로 한 연구용 벤치마크입니다.",
            "통과해도 주가 미반영도·수익률·거래비용 검증 전에는 투자 사용을 허용하지 않습니다.",
        ],
    }
    output_dir.mkdir(parents=True, exist_ok=True)
    for name, payload in {
        "v1_external_summary.json": summary,
        "v1_external_ranking.json": ranking,
        "v1_external_scenarios.json": scenarios,
        "v1_external_model_lock.json": lock_result,
        "v1_external_financial_status.json": financial_status,
    }.items():
