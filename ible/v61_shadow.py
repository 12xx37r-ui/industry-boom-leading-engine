...
def run_v61(root:Path,output_dir:Path,run_date:str|None=None,v50_output_dir:Path|None=None,v60_output_dir:Path|None=None)->dict[str,Any]:
    ...
    # The monthly snapshot is intentionally immutable within a month and is used only
    # for prospective validation. V7 operational output must use the current run's
    # snapshot when available; fall back to the legacy monthly file for compatibility.
    current_run_path=v50/'v50_current_run_snapshot.json'
    source=load_json(current_run_path if current_run_path.is_file() else v50/'v50_current_monthly_snapshot.json'); v50sum=load_json(v50/'v50_run_summary.json'); v60sum=load_json(v60/'v60_run_summary.json'); as_of=str(source['as_of']); registry_path=root/'prospective_history/v70_operational_registry.json'; registry=_load_registry(registry_path); reference=_find_three_month_reference(root,registry,as_of); decisions=_decision_rows(root,source,policy,reference)
