...
def run_v61(root:Path,output_dir:Path,run_date:str|None=None,v50_output_dir:Path|None=None,v60_output_dir:Path|None=None)->dict[str,Any]:
    ...
    required=[v50/'v50_current_run_snapshot.json',v50/'v50_run_summary.json',v60/'v60_run_summary.json']; missing=[str(p) for p in required if not p.is_file()]
    if missing: raise V61Error(f'required upstream outputs missing: {missing}')
    # V70 is an operational snapshot: consume this execution's V50 snapshot.
    # The monthly snapshot remains immutable for prospective validation/audit only.
    source=load_json(v50/'v50_current_run_snapshot.json'); v50sum=load_json(v50/'v50_run_summary.json'); v60sum=load_json(v60/'v60_run_summary.json'); as_of=str(source['as_of']); registry_path=root/'prospective_history/v70_operational_registry.json'; registry=_load_registry(registry_path); reference=_find_three_month_reference(root,registry,as_of); decisions=_decision_rows(root,source,policy,reference)
