...
class V62ReleaseTests(unittest.TestCase):
    ...
    def test_workflow_is_single_github_path_and_no_legacy_networks(self):
        ...
        self.assertIn("tests.test_v62_release", workflow)
    ...
    def test_gas_dashboard_reads_committed_v61_outputs(self):
        code = (ROOT / "google_apps_script/Code.gs").read_text(encoding="utf-8")
        html = (ROOT / "google_apps_script/Index.html").read_text(encoding="utf-8")
        self.assertIn("outputs/v61_prospective_policy/v61_dashboard_payload.json", code)
        self.assertIn("outputs/v60_champion_challenger/v60_champion_challenger_comparison.json", code)
        self.assertIn("GITHUB_OWNER", code)
        self.assertIn("GITHUB_TOKEN", code)
        self.assertIn("산업 TOP20", html)
        self.assertIn("Champion", html)
        self.assertIn("Challenger", html)
        self.assertIn("미래 6·12·24개월 성적표", html)

    def test_dashboard_does_not_expose_token_to_client_payload(self):
        code = (ROOT / "google_apps_script/Code.gs").read_text(encoding="utf-8")
        self.assertIn("private_token_used", code)
        self.assertNotIn("token: source.token", code)
