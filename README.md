...
- SEC Company Facts 86개 기업을 4개 워커로 병렬 수집합니다.
- HTTP 제한시간 12초, 재시도 1회로 제한해 장애 시 장시간 정지하지 않습니다.
- SEC 광범위 접속실패는 초기에 감지해 명확한 오류로 종료합니다.
- OpenDART는 첫 실행 기본값이 `false`이며 필요할 때만 별도 보강합니다.
- 실행 로그에 SEC/DART 진행률과 단계별 상태를 실시간 출력합니다.

첫 검증 실행은 `include_dart=false`로 실행하세요. 성공 후 한국 보강이 필요할 때만 `true`로 다시 실행합니다.
